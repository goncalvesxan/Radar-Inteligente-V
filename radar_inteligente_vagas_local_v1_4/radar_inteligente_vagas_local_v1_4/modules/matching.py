...
def analyze_match(job_text: str, profile: dict) -> dict:
    skills = profile.get("competencias_fortes", []) or []
    job_low = (job_text or "").lower()
    found, missing = [], []
    for s in skills:
        if str(s).lower() in job_low:
            found.append(s)
        else:
            missing.append(s)
    area_hits = []
    for a in profile.get("preferencias", {}).get("areas", []) or []:
        if str(a).lower() in job_low:
            area_hits.append(a)
    ...
    stop = {"para", "como", "com", "uma", "dos", "das", "que", "and", "the", "you", "are", "vaga", "empresa"}
    tokens = [x for x in tokenize(job_text) if len(x) > 3 and x not in stop]
    common = [w for w, _ in Counter(tokens).most_common(25)]
    return {
        "score": score,
