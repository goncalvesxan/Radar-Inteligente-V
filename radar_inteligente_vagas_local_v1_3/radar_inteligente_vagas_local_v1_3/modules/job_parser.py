...
def extract_from_public_url(url: str) -> dict:
    """Extrai detalhes de páginas públicas. Para LinkedIn, retorna orientação de captura manual/local."""
    if not url:
...
def infer_job_fields(text: str, url: str = "") -> dict:
    ...
    if lines:
        cargo = lines[0][:120]
    patterns = {
        "empresa": [r"Empresa[:\-]\s*(.+)", r"Company[:\-]\s*(.+)"],
        "local": [r"Local[:\-]\s*(.+)", r"Localização[:\-]\s*(.+)", r"Location[:\-]\s*(.+)"]
    }
